...
public class Table extends ObjetGen {
	...
	public void drawObjet(GL2 gl, GLUT glut, Texture text_table) {
	    ...
	    gl.glColor3d(1, 0, 0); // set the color of the quad
			gl.glVertex3d(posX + 3, posY-2, 1); // Top Left
			gl.glVertex3d(posX + 3, posY-3, 1); // Top Right
			gl.glVertex3d(posX + 6,posY -3, 1); // Bottom Right
		...
		gl.glBegin(GL2.GL_QUADS);
		gl.glColor3d(1, 0, 0); // set the color of the quad
			gl.glVertex3d(posX + 3,posY -2, 0); // Top Left
			gl.glVertex3d( posX + 3,posY -3, 0); // Top Right
			gl.glVertex3d(posX + 3, posY-3, 1); // Bottom Right
			gl.glVertex3d(posX + 3,posY -2, 1); // Bottom Left
		gl.glEnd();
		...
		gl.glColor3d(1, 0, 0); // set the color of the quad
			...
			gl.glVertex3d(posX + 6,posY -3, 0); // Top Right
			gl.glVertex3d(posX + 6,posY -3, 1); // Bottom Right
			gl.glVertex3d(posX + 6,posY -2, 1); // Bottom Left
		gl.glEnd();
